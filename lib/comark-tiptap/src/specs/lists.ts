...
export const listItemSpec: NodeSpec = {
  ...
  toComark(node: JSONContent, h: ComarkHelpers): ComarkElement {
    ...
    // `hasNoHtmlAttrs` collapses missing-vs-`{}` so a paragraph
    // round-tripped through DOM (where PM fills the default `{}`) is
    // still recognised as "attrless" and gets flattened to the tight
    // form.
  ...
  fromComark(el: ComarkElement, h: ComarkHelpers): JSONContent {
    ...
    // into paragraphs and passes block elements through. Delegating
    // keeps a single source of truth for "what's inline" rather than
    // duplicating the rule with a hardcoded `br | img` set.
...
export const orderedListSpec: NodeSpec = {
  ...
  toComark(node: JSONContent, h: ComarkHelpers): ComarkElement {
    ...
    // stability. Mirror that on output so
    // `parse(md) === toComark(fromComark(...))`.
